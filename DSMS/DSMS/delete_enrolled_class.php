...
if($query_run)
        {
            ...
            header("Location:indiv_class.php? id=$c_id");
        }
        else
        {
            echo '<script> alert("Data Not Updated"); </script>';
        }
    }else if (isset($_GET['deleteRemoval'])){

        $id = $_GET['deleteRemoval'];
        
        $_SESSION['StudRemoval_updated']=true;
        $sql2 = "SELECT * FROM removal_grade  left join class using(class_id) WHERE removal_id = $id";
        echo $sql2;
            $result2 = mysqli_query($con,$sql2);
            if ($result2->num_rows > 0){
                $row2 = $result2->fetch_assoc();
                $c_id = $row2["stud_num"];
                $class_num = $row2["course_num"];
